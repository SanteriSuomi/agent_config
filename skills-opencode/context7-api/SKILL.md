...
Response: array of `results` with `id` (format `/owner/repo`), `title`, `description`, `versions`.
...
- `libraryId` — from search results (e.g. `/facebook/react`)
...
Response: `codeSnippets[]` + `infoSnippets[]`. Default is `text/plain`; add `&type=json` for structured JSON.

### Additional Parameters

- `fast=true` — skip LLM reranking (lower latency, lower relevance)
- `researchMode=true` — deeper research (requires API key, weekly quota)
...
2. **Extract** `libraryId` from results (format: `/owner/repo`)
