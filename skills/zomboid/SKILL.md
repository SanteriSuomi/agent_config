...
**`zomboid-butler` sidecar** (renamed from zomboid-backup, same `~/mediaserver/zomboid-backup/backup.py` mount, compose service `zomboid-butler`): hourly cycle = RCON `save` → SQLite-API consistent copies → `world-hourly-<ts>.tar.gz` (keep 24, `/opt/dockerdata/zomboid/backups/`) + memory anon/file telemetry + GC digest (parses `config/gc.log`, appends to `backups/gc-history.log`, "pending" until first server restart with GC logging) + workshop update check + RCON health (ntfy on stall). 60s tick = **emergency valve** (`EMERGENCY_TOTAL_PCT=90` of container limit OR `EMERGENCY_ANON_GB=10` → servermsg 2-min warning (repeat T-30s) → save → docker-API restart → verify 42.20.3 → ntfy; **RCON dead at trigger = instant restart** (freeze signature; docker API works when frozen); 30-min cooldown) + **dynamic update restarts** (windows removed): pending + empty server → restart now; pending + "initiating a connection" in docker logs since last tick → announced restart; 12h cooldown. State: `backups/update-state.json`. Notifications go DIRECTLY to ntfy container IP via docker API lookup (Caddy :80 308-redirects to unverifiable https — `http://<ntfy-ip>:80/<topic>` works).

**STAGED (2026-09-07, pending `docker compose up -d zomboid` — valve restarts via docker API do NOT apply compose config)**: `GC_CONFIG=ZGC` + `MAX_RAM=12G` + `mem_limit: 20g` + `JAVA_TOOL_OPTIONS=-XX:+UseCompactObjectHeaders` (env, NOT vmArgs — the entrypoint sed `s/-XX:+Use.*/...` would clobber any `-XX:+Use*` flag added to the json) + rotated `-Xlog:gc:file=/home/steam/Zomboid/gc.log:time,uptime:filecount=5,filesize=10M` in json + mod `Tempo_PerfKit`/`3736629791` (B42 perf: dense-zombie render LOD, chunk streaming; successor of the pulled Tempo_PerfKit_42203_Local lineage).
