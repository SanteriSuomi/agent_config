...
**Workshop update-sync (same sidecar, same hourly cycle)**: parses active workshop IDs from INI `WorkshopItems=` (**semicolon-separated**, not comma), diffs installed `timeupdated` (steamapps `appworkshop_108600.acf`, mounted ro) against Steam Web API `GetPublishedFileDetails` → `update-state.json` in backups dir. Restart rule: pending + 0 players → immediate restart; pending + players → forced at hours `UPDATE_RESTART_HOURS=6,18` (once per hour-key); cooldown `UPDATE_COOLDOWN_HOURS=12`. Restarts via docker.sock (stdlib unix-socket HTTP): announce (quoted servermsg) → save → `POST /containers/zomboid/restart?t=120` → poll health → verify `version=42.20.3` from logs API → notify ntfy. Notifications go DIRECTLY to the ntfy container IP via docker API lookup (Caddy on :80 308-redirects to https with unverifiable internal cert — `http://<ntfy-container-ip>:80/<topic>` works). Version mismatch → ntfy priority-5 alert. State: `/opt/dockerdata/zomboid/backups/update-state.json`.
